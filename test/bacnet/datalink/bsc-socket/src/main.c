...
static void
wait_for_srv_specific_socket_state(BSC_SOCKET *s, BSC_SOCKET_STATE st)
{
    int i;
    call_maintenance_timer(1, 0);
    while (1) {
...
static void wait_specific_sock_ev(sock_ev_t *ev, BSC_SOCKET_EVENT wait_ev)
{
    ...
    while (1) {
        ...
        bws_dispatch_lock();
        debug_printf(
            "wait_specific: wait_sock_ev ev = %p awaited_ev %d "
            "received_ev %d\n",
        ...
        if (ev->ev_code == wait_ev) {
            ...
            debug_printf(
                "wait_specific: event is ignored wait_sock_ev ev = %p "
...
#endif
{
    ...
    bws_dispatch_lock();
    zassert_equal(
        cli_socks[0].state == BSC_SOCK_STATE_CONNECTED &&
...
#endif
{
    ...
    bsc_init_ctx_cfg(
        BSC_SOCKET_CTX_INITIATOR, &client_cfg, BSC_WEBSOCKET_DIRECT_PROTOCOL,
        BACNET_WEBSOCKET_SERVER_PORT, BSC_NETWORK_IFACE, NULL, sizeof(ca_cert),
