...
#define BACNET_NODE_LOCAL_HUB_PORT 39005
#define BACNET_NODE_LOCAL_DIRECT_PORT 39006
#define BACNET_NODE_LOCAL_HUB_PORT2 39007
#define BACNET_NODE_LOCAL_DIRECT_PORT2 39008
#define BACNET_NODE_LOCAL_HUB_PORT3 39009
#define BACNET_NODE_LOCAL_DIRECT_PORT3 39010
...
{
    debug_printf(
        "wait_node_ev() >>> ev = %p ev->e = %p ev->ev = %d wait_ev = "
        "%d, node = %p\n",
    ...
    if (ev->ev == wait_ev && ev->node == node) {
        debug_printf(
            "wait_node_ev() got event %d and reset ev %p ev->e %p\n", ev->ev,
            ev, ev->e);
        ev->ev = -1;
        ...
        bws_dispatch_unlock();
        debug_printf(
            "wait_node_ev() <<< ret = true, ev = %p ev->ev = %d "
            "wait_ev = %d, node = %p\n",
        ...
        bws_dispatch_unlock();
        debug_printf(
            "wait_node_ev() <<< ret = false,  ev = %p ev->e %p ev->ev "
            "= %d wait_ev = %d, node = %p\n",
            ev, ev->e, ev->ev, wait_ev, node);
        return false;
    }
...
{
    debug_printf(
        "wait_specific_node_ev() >>> ev = %p, wait_ev %d, node = %p\n", ev,
        wait_ev, node);
    call_maintenance_timer(1, 0);
    while (1) {
        while (!bsc_event_timedwait(ev->e, WAIT_EVENT_MS)) {
        ...
        if (ev->ev == wait_ev && ev->node == node) {
            debug_printf(
                "wait_specific_node_ev() got event %d and reset ev %p "
                "ev->e %p\n",
                ev->ev, ev, ev->e);
            ev->ev = -1;
            // reset event if it was signalled while we were blocked in call
...
#endif
{
    ...
    zassert_equal(
        message.payload.result.error_code, ERROR_CODE_HEADER_NOT_UNDERSTOOD,
        NULL);
    ...
    zassert_equal(
        message.payload.advertisiment.support,
...
#endif
{
    ...
    ret = memcmp(
        &node_vmac2.address[0], &node_ev3.dest.address[0],
...
#endif
{
    ...
    zassert_equal(
        message.payload.result.error_class, ERROR_CLASS_COMMUNICATION, NULL);
    zassert_equal(
        message.payload.result.error_code,
...
#endif
{
    ...
    ret = bsc_node_switch_start(
        NULL, 0, NULL, 0, NULL, 0, 0, NULL, NULL, NULL, 0, 0, 0, 0, 0, 0, 0, 0,
    ...
    zassert_equal(ret == BSC_SC_BAD_PARAM, true, 0);
    ret = bsc_node_switch_start(
        ca_cert, sizeof(ca_cert), node_cert, sizeof(node_cert), node_key,
        sizeof(node_key), BACNET_NODE_LOCAL_DIRECT_PORT, NULL, &node_uuid,
        &node_vmac, MAX_BVLC_LEN, MAX_NDPU_LEN, BACNET_TIMEOUT, BACNET_TIMEOUT,
        BACNET_TIMEOUT, BACNET_TIMEOUT, BACNET_TIMEOUT, false, false,
        node_switch_event, NULL, &sh[0]);

    zassert_equal(ret == BSC_SC_BAD_PARAM, true, 0);
    for (i = 0; i < sizeof(sh) / sizeof(sh[0]); i++) {
        res[i] = bsc_node_switch_start(
            ca_cert, sizeof(ca_cert), node_cert, sizeof(node_cert), node_key,
            sizeof(node_key), BACNET_NODE_LOCAL_DIRECT_PORT + i, NULL,
            &node_uuid, &node_vmac, MAX_BVLC_LEN, MAX_NDPU_LEN, BACNET_TIMEOUT,
            BACNET_TIMEOUT, BACNET_TIMEOUT, BACNET_TIMEOUT, BACNET_TIMEOUT,
            false, true, node_switch_event, NULL, &sh[i]);
    }
    ...
    zassert_equal(ret == true, true, 0);
    ret = bsc_node_switch_start(
        NULL, sizeof(ca_cert), node_cert, sizeof(node_cert), node_key,
        sizeof(node_key), BACNET_NODE_LOCAL_DIRECT_PORT, NULL, &node_uuid,
        &node_vmac, MAX_BVLC_LEN, MAX_NDPU_LEN, BACNET_TIMEOUT, BACNET_TIMEOUT,
    ...
    ret = bsc_node_switch_start(
        ca_cert, sizeof(ca_cert), node_cert, sizeof(node_cert), node_key,
        sizeof(node_key), BACNET_NODE_LOCAL_DIRECT_PORT + 1, NULL, &node_uuid,
        &node_vmac, MAX_BVLC_LEN, MAX_NDPU_LEN, BACNET_TIMEOUT, BACNET_TIMEOUT,
        BACNET_TIMEOUT, BACNET_TIMEOUT, BACNET_TIMEOUT, true, false,
        node_switch_event, NULL, &sh[0]);
    zassert_equal(ret == BSC_SC_SUCCESS, true, 0);
