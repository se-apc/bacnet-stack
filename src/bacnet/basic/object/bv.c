...
static binary_value_write_present_value_callback Binary_Value_Write_Present_Value_Callback;

/* These three arrays are used by the ReadPropertyMultiple handler */
static const int Binary_Value_Properties_Required[] = {
    PROP_OBJECT_IDENTIFIER, PROP_OBJECT_NAME, PROP_OBJECT_TYPE,    PROP_PRESENT_VALUE,
    PROP_STATUS_FLAGS,      PROP_EVENT_STATE, PROP_OUT_OF_SERVICE, -1};
 ...
 */
void Binary_Value_Property_Lists(const int **pRequired, const int **pOptional, const int **pProprietary) {
  if (pRequired) {
 ...
 */
static struct object_data *Binary_Value_Object(uint32_t object_instance) { return Keylist_Data(Object_List, object_instance); }
 ...
 */
unsigned Binary_Value_Count(void) { return Keylist_Count(Object_List); }
 ...
 */
unsigned Binary_Value_Instance_To_Index(uint32_t object_instance) { return Keylist_Index(Object_List, object_instance); }
 ...
 */
static void Binary_Value_Present_Value_COV_Detect(struct object_data *pObject, BACNET_BINARY_PV value) {
  if (pObject) {
 ...
 */
bool Binary_Value_Encode_Value_List(uint32_t object_instance, BACNET_PROPERTY_VALUE *value_list) {
  bool status = false;
 ...
 */
bool Binary_Value_Present_Value_Set(uint32_t object_instance, BACNET_BINARY_PV value) {
  bool status = false;
  struct object_data *pObject;
 ...
 * @return  true if values are within range and present-value is set.
 */
static bool Binary_Value_Present_Value_Write(uint32_t object_instance, BACNET_BINARY_PV value, BACNET_ERROR_CLASS *error_class,
    ...
    if (value <= MAX_BINARY_PV) {
      if (pObject->Write_Enabled) {
        ...
        if (pObject->Out_Of_Service) {
          ...
          Binary_Value_Write_Present_Value_Callback(object_instance, old_value, value);
        }
        status = true;
      } else {
 ...
 */
bool Binary_Value_Polarity_Set(uint32_t object_instance, BACNET_POLARITY polarity) {
  bool status = false;
  struct object_data *pObject;
 ...
 */
static int Binary_Value_Event_Time_Stamps_Encode(uint32_t object_instance, BACNET_ARRAY_INDEX index, uint8_t *apdu) {
...
int Binary_Value_Read_Property(BACNET_READ_PROPERTY_DATA *rpdata) {
  ...
  case PROP_PRESENT_VALUE:
    apdu_len = encode_application_enumerated(&apdu[0], Binary_Value_Present_Value(rpdata->object_instance));
    break;
  ...
  case PROP_OUT_OF_SERVICE:
    ...
    break;
  case PROP_POLARITY:
    apdu_len = encode_application_enumerated(&apdu[0], Binary_Value_Polarity(rpdata->object_instance));
    break;
  case PROP_RELIABILITY:
    apdu_len = encode_application_enumerated(&apdu[0], Binary_Value_Reliability(rpdata->object_instance));
    break;
  ...
  case PROP_ACTIVE_TEXT:
    characterstring_init_ansi(&char_string, Binary_Value_Active_Text(rpdata->object_instance));
    apdu_len = encode_application_character_string(&apdu[0], &char_string);
    break;
  case PROP_INACTIVE_TEXT:
    characterstring_init_ansi(&char_string, Binary_Value_Inactive_Text(rpdata->object_instance));
    apdu_len = encode_application_character_string(&apdu[0], &char_string);
    break;
#if defined(INTRINSIC_REPORTING) && (BINARY_VALUE_INTRINSIC_REPORTING)
  ...
  case PROP_EVENT_TIME_STAMPS:
    apdu_len = bacnet_array_encode(rpdata->object_instance, rpdata->array_index, Binary_Value_Event_Time_Stamps_Encode,
  ...
  if ((apdu_len >= 0) && (rpdata->object_property != PROP_PRIORITY_ARRAY) && (rpdata->array_index != BACNET_ARRAY_ALL)) {
    rpdata->error_class = ERROR_CLASS_PROPERTY;
...
bool Binary_Value_Write_Property(BACNET_WRITE_PROPERTY_DATA *wp_data) {
  ...
  /* Only array properties can have array options. */
  if ((wp_data->object_property != PROP_PRIORITY_ARRAY) && (wp_data->array_index != BACNET_ARRAY_ALL)) {
    wp_data->error_class = ERROR_CLASS_PROPERTY;
  ...
  case PROP_PRESENT_VALUE:
    ...
    if (status) {
      status = Binary_Value_Present_Value_Write(wp_data->object_instance, value.type.Enumerated, &wp_data->error_class,
                                                &wp_data->error_code);
    }
    break;
  case PROP_OUT_OF_SERVICE:
    ...
    if (status) {
      Binary_Value_Out_Of_Service_Set(wp_data->object_instance, value.type.Boolean);
    }
    break;
  case PROP_POLARITY:
    status = write_property_type_valid(wp_data, &value, BACNET_APPLICATION_TAG_ENUMERATED);
    if (status) {
      if (value.type.Enumerated < MAX_POLARITY) {
        Binary_Value_Polarity_Set(wp_data->object_instance, (BACNET_POLARITY)value.type.Enumerated);
      } else {
        status = false;
        wp_data->error_class = ERROR_CLASS_PROPERTY;
        wp_data->error_code = ERROR_CODE_VALUE_OUT_OF_RANGE;
  ...
  case PROP_ALARM_VALUE:
    ...
    if (status) {
      if (value.type.Enumerated <= MAX_BINARY_PV) {
        Binary_Value_Alarm_Value_Set(wp_data->object_instance, (BACNET_BINARY_PV)value.type.Enumerated);
      } else {
        status = false;
        wp_data->error_class = ERROR_CLASS_PROPERTY;
        wp_data->error_code = ERROR_CODE_VALUE_OUT_OF_RANGE;
...
#endif
  default:
    if (property_lists_member(Binary_Value_Properties_Required, Binary_Value_Properties_Optional,
                              Binary_Value_Properties_Proprietary, wp_data->object_property)) {
      wp_data->error_class = ERROR_CLASS_PROPERTY;
 ...
 */
void Binary_Value_Write_Present_Value_Callback_Set(binary_value_write_present_value_callback cb) {
...
uint32_t Binary_Value_Create(uint32_t object_instance) {
  ...
  if (!pObject) {
    ...
    if (pObject) {
      ...
      handler_get_event_information_set(Object_Type, Binary_Value_Event_Information);
      /* Set handler for AcknowledgeAlarm function */
      handler_alarm_ack_set(Object_Type, Binary_Value_Alarm_Ack);
      /* Set handler for GetAlarmSummary Service */
      handler_get_alarm_summary_set(Object_Type, Binary_Value_Alarm_Summary);
#endif
      /* add to list */
 ...
 * @return object found in the list, or NULL if not found
 */
static struct object_data *Binary_Value_Object_Index(int index) { return Keylist_Data_Index(Object_List, index); }
...
int Binary_Value_Event_Information(unsigned index, BACNET_GET_EVENT_INFORMATION_DATA *getevent_data) {
  ...
  if ((IsActiveEvent) || (IsNotAckedTransitions)) {
    ...
    getevent_data->objectIdentifier.instance = Binary_Value_Index_To_Instance(index);
    /* Event State */
...
int Binary_Value_Alarm_Ack(BACNET_ALARM_ACK_DATA *alarmack_data, BACNET_ERROR_CODE *error_code) {
  struct object_data *pObject = NULL;
...
int Binary_Value_Alarm_Summary(unsigned index, BACNET_GET_ALARM_SUMMARY_DATA *getalarm_data) {
  struct object_data *pObject = Binary_Value_Object_Index(index);
  ...
  if (pObject) {
    ...
    if ((pObject->Event_State != EVENT_STATE_NORMAL) && (pObject->Notify_Type == NOTIFY_ALARM)) {
      ...
      getalarm_data->objectIdentifier.instance = Binary_Value_Index_To_Instance(index);
      /* Alarm State */
